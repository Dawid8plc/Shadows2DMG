...
void FullScreenVS(in float3 InPos : POSITION, in float2 InTex : TEXCOORD0, out float4 Position : POSITION, out float2 TexCoord : TEXCOORD0)
{
    // Offset the position by half a pixel to correctly align texels to pixels
    Position = float4(InPos,1) + 0.5f* float4(1.0f/renderTargetSize.x, -1.0f/renderTargetSize.y, 0, 0);
    TexCoord = InTex;
}

float4 ComputeDistancesPS(in float4 Position : POSITION, in float2 TexCoord : TEXCOORD0) : COLOR0
{
...
float4 DistortPS(in float4 Position : POSITION, in float2 TexCoord : TEXCOORD0) : COLOR0
{
...
float4 DrawShadowsPS(in float4 Position : POSITION, in float2 TexCoord : TEXCOORD0) : COLOR0
{
...
float4 BlurHorizontallyPS(in float4 Position : POSITION, in float2 TexCoord : TEXCOORD0) : COLOR0
{
	  float sum=0;
...
float4 BlurVerticallyPS(in float4 Position : POSITION, in float2 TexCoord : TEXCOORD0) : COLOR0
{
	  float sum=0;
	  float distance = tex2D( inputSampler, TexCoord).b;
